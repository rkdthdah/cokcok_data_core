...
def addFeature(all_data):
    for data in all_data:
        ...
        for i in range(1, len(data)):
            if data.at[i, CONST.PITCH] < (data.at[i - 1, CONST.PITCH] - 3): data.at[i, CONST.PITCH] += 2 * math.pi
            if data.at[i, CONST.ROLL] < (data.at[i - 1, CONST.ROLL] - 3): data.at[i, CONST.ROLL] += 2 * math.pi
            if data.at[i, CONST.YAW] < (data.at[i - 1, CONST.YAW] - 3): data.at[i, CONST.YAW] += 2 * math.pi
        ...
        data[CONST.RVP] = (data[CONST.PITCH] - data[CONST.PITCH].shift(1)) * 180 / math.pi * 50
        data[CONST.RVP] = data[CONST.RVP].fillna(0)

        data[CONST.RVR] = (data[CONST.ROLL] - data[CONST.ROLL].shift(1)) * 180 / math.pi * 50
        data[CONST.RVR] = data[CONST.RVR].fillna(0)

        data[CONST.RVY] = (data[CONST.YAW] - data[CONST.YAW].shift(1)) * 180 / math.pi * 50
        data[CONST.RVY] = data[CONST.RVY].fillna(0)

        data[CONST.ACCX] = -data[CONST.ACCX]
        data[CONST.ACCY] = -data[CONST.ACCY]

        data[CONST.RVP] = -data[CONST.RVP]
        data[CONST.RVR] = -data[CONST.RVR]
    ...
    for name in list:
        ...
        for file_name in os.listdir(folder_path):
            ...
            if file_name.endswith('.json') and file_name.startswith(name[1]):
                ...
                for entry in data:
                    new_entry = {
                        CONST.ACCX: float(entry['userAcceleration']['x']),
                        CONST.ACCY: float(entry['userAcceleration']['y']),
                        CONST.ACCZ: float(entry['userAcceleration']['z']),
                        CONST.PITCH: float(entry['attitude']['pitch']),
                        CONST.ROLL: float(entry['attitude']['roll']),
                        CONST.YAW: float(entry['attitude']['yaw'])
                    }
                ...
                same_name.append(cutData(addFeature([df]))[0])
...
ex_list = [['expert', 'A남10'], 
            ['expert', 'A남11'], 
            ['expert', 'A남13']]
ex_motion_list = [['expertmotion', 'A남10'], 
            ['expertmotion', 'A남11'], 
            ['expertmotion', 'A남13']]

# data 읽기 및 dataframe으로 변환
ex_cut_data = getCSVList(ex_list)

# 이 부분에 차원 추가가 들어간다. def 차원추가(cutted_data) -> def normalize(it)
# ex_add_data = addFeature(ex_cut_data)

# 전문가 평균
ex_avg = avgData(ex_cut_data) # ex_add_data
# 전문가 평균 csv 저장
script_dir = os.path.dirname(os.path.abspath(__file__))
csv_file_name = '/ex_avg_raw.csv'
ex_avg[0].to_csv(CONST.CURRENT_PATH + csv_file_name, index=False)
...
plt.subplot(2, 1, 1)
plt.plot(ex_avg[0].index, ex_avg[0][CONST.ACCX], label='X')
plt.plot(ex_avg[0].index, ex_avg[0][CONST.ACCY], label='Y')
plt.plot(ex_avg[0].index, ex_avg[0][CONST.ACCZ], label='Z')
plt.xlabel('Index')
plt.ylabel('acceleration')
plt.legend()

plt.subplot(2, 1, 2)
plt.plot(ex_avg[0].index, ex_avg[0][CONST.RVP], label='Pitch')
plt.plot(ex_avg[0].index, ex_avg[0][CONST.RVR], label='Roll')
plt.plot(ex_avg[0].index, ex_avg[0][CONST.RVY], label='Yaw')
plt.xlabel('Index')
plt.ylabel('Gyroscope')
plt.legend()
plt.show()
# window, 정규화 적용 된 전문가 평균, 표준 편차 csv로 저장 시작
script_dir = os.path.dirname(os.path.abspath(__file__))
csv_file_name = '/ex_avg.csv'
ex_avg_windowed_data[0].to_csv(CONST.CURRENT_PATH + csv_file_name, index=False)
...
with open(CONST.CURRENT_PATH + csv_file_name, 'w', newline='') as csvfile:
